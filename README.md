<picture>
  <source media="(prefers-color-scheme: dark)" srcset="./banner-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="./banner-light.svg">
  <img src="./banner-light.svg" alt="Hey, I'm Felix :)">
</picture>

> *Always learning, always exploring.*

I'm a software engineer graduate from [Marcy Lab School](https://www.marcylabschool.org/) in Brooklyn, New York, where I've built full-stack web applications with the PERN stack. I care about writing clean, purposeful code — and I bring the same energy to engineering that I bring to everything else: all in.

When I'm not coding, you'll find me deep in a game, lost in a book, sketching ideas, or exploring the city I've called home my whole life.

---

### 「 Featured Projects 」

<a href="https://github.com/NYC-Subtack-chat/NYC-Transit-App"><picture><source media="(prefers-color-scheme: dark)" srcset="./subtrack-dark.svg"><source media="(prefers-color-scheme: light)" srcset="./subtrack-light.svg"><img src="./subtrack-light.svg"></picture></a>

In progress · NYC transit app, built with a team
`Swift` `Node.js` `PostgreSQL/Supabase` `Socket.io` `MTA API`

<a href="https://github.com/F-NAN-AI-ENGINEERING-RESIDENCY/compass"><picture><source media="(prefers-color-scheme: dark)" srcset="./compass-dark.svg"><source media="(prefers-color-scheme: light)" srcset="./compass-light.svg"><img src="./compass-light.svg"></picture></a>

Complete · Civic tech capstone — virtual classroom app, Product Lead
`Python` `FastAPI` `PostgreSQL` `React` `WebSockets` `Gemini`

<a href="https://github.com/felixvargas7/full-stack-project-remix-felixvargas7/tree/feature/final-revisions"><picture><source media="(prefers-color-scheme: dark)" srcset="./workstarter-dark.svg"><source media="(prefers-color-scheme: light)" srcset="./workstarter-light.svg"><img src="./workstarter-light.svg"></picture></a>

Complete · PERN workout tracker — primary portfolio project
`React` `Express` `PostgreSQL` `Node.js`

<a href="https://github.com/felix-majeed-mls/mod-4-project"><picture><source media="(prefers-color-scheme: dark)" srcset="./majflix-dark.svg"><source media="(prefers-color-scheme: light)" srcset="./majflix-light.svg"><img src="./majflix-light.svg"></picture></a>

Complete · TV/movie discovery app
`JavaScript` `Vite` `TVMaze API`

<picture>
  <source media="(prefers-color-scheme: dark)" srcset="./tech-stack-dark.svg">
  <source media="(prefers-color-scheme: light)" srcset="./tech-stack-light.svg">
  <img src="./tech-stack-light.svg" alt="Tech stack">
</picture>

---

## 📬 「 Let's Connect 」

<a href="mailto:felix70529@gmail.com"><picture><source media="(prefers-color-scheme: dark)" srcset="./email-dark.svg"><source media="(prefers-color-scheme: light)" srcset="./email-light.svg"><img src="./email-light.svg"></picture></a>
<a href="https://www.linkedin.com/in/felixgvargas/"><picture><source media="(prefers-color-scheme: dark)" srcset="./linkedin-dark.svg"><source media="(prefers-color-scheme: light)" srcset="./linkedin-light.svg"><img src="./linkedin-light.svg"></picture></a>
